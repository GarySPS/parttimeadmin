  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3">
        <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900">KYC Review Queue</h1>
            ...
            return (
              <div key={app.id} className="p-4 flex flex-col gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-slate-900">{app.profiles?.contact_username || 'Unknown'}</p>
                ...
                <div className="mt-2 pt-3 border-t border-slate-100 flex gap-2">
                  <form action={rejectKyc} className="flex-1">
                    ...
                    <button type="submit" className="w-full py-2.5 bg-rose-50 text-rose-600 border border-rose-200 hover:bg-rose-100 rounded-xl text-xs font-bold shadow-sm">Reject</button>
                  </form>
                  <form action={approveKyc} className="flex-1">
                    <input type="hidden" name="id" value={app.id} />
                    <input type="hidden" name="userId" value={app.user_id} />
                    <input type="hidden" name="tier" value={app.tier} />
                    <button type="submit" className="w-full py-2.5 bg-emerald-50 text-emerald-600 border border-emerald-200 hover:bg-emerald-100 rounded-xl text-xs font-bold shadow-sm">Approve</button>
                  </form>
                </div>
              </div>
            );
                  ...
                  <tr key={app.id} className="hover:bg-slate-50/50">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="font-semibold text-slate-900">{app.profiles?.contact_username || 'Unknown'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <form action={rejectKyc}>
                          <input type="hidden" name="id" value={app.id} />
                          <input type="hidden" name="userId" value={app.user_id} />
                          <button type="submit" className="px-4 py-2 bg-rose-50 text-rose-600 border border-rose-200 hover:bg-rose-100 rounded-xl text-xs font-bold transition-colors">Reject</button>
                        </form>
                        <form action={approveKyc}>
                          <input type="hidden" name="id" value={app.id} />
                          <input type="hidden" name="userId" value={app.user_id} />
                          <input type="hidden" name="tier" value={app.tier} />
                          <button type="submit" className="px-4 py-2 bg-emerald-50 text-emerald-600 border border-emerald-200 hover:bg-emerald-100 rounded-xl text-xs font-bold transition-colors">Approve</button>
                        </form>
                      </div>
                    </td>
                  </tr>
