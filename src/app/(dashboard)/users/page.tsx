  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 sm:mb-6 gap-3">
        <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900">အသုံးပြုသူများ နှင့် KYC</h1>
        <span className="px-3 py-1.5 bg-slate-100 text-slate-600 rounded-full text-xs sm:text-sm font-bold border border-slate-200 shadow-sm">
          စုစုပေါင်း အသုံးပြုသူ {count || 0} ဦး
        </span>
      </div>
      ...
      <form method="GET" action="/users" className="mb-4 sm:mb-6 flex gap-2">
        <div className="relative flex-1">
          <input
            ...
            placeholder="အသုံးပြုသူအမည်၊ ဖုန်း၊ အီးမေးလ် သို့မဟုတ် handle ဖြင့် ရှာရန်..."
              ...
              <div key={profile.id} className="p-3 sm:p-4 hover:bg-slate-50/50 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-xs font-semibold text-slate-500 capitalize">{profile.role || 'အသုံးပြုသူ'}</span>
                  </div>
                </div>
                ...
                {(profile.contact_app || profile.contact_username) && (
                  <div className="mt-3 flex items-center gap-2 p-2 bg-slate-50 rounded-lg border border-slate-100 text-xs">
                    <span className="font-bold text-slate-600">{profile.contact_app || 'ဆက်သွယ်ရန်'}:</span>
                    <span className="text-slate-500 truncate">{profile.contact_username || 'မဖြည့်ထားပါ'}</span>
                  </div>
                )}
                ...
                <div className="mt-3 flex items-end justify-between flex-wrap gap-3">
                  <div className="flex flex-col gap-2">
                    <div className="flex gap-2">
                      <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider border ${
                        ...
                        {profile.is_verified ? 'အတည်ပြုပြီး' : 'အတည်မပြုရသေးပါ'}
                      </span>
                      <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider border ${
                        profile.is_blocked ? 'bg-rose-50 text-rose-600 border-rose-200' : 'bg-emerald-50 text-emerald-600 border-emerald-200'
                      }`}>
                        {profile.is_blocked ? 'ပိတ်ပင်ထားသည်' : 'အသုံးပြုနိုင်သည်'}
                      </span>
                    </div>

                    {(idCardUrl || selfieUrl) && (
                      <div className="flex gap-2 mt-1">
                        {idCardUrl && <DocumentModal url={idCardUrl} label="မှတ်ပုံတင် ကြည့်ရန်" className="px-2 py-1 bg-slate-100 text-slate-700 border border-slate-200 rounded text-[10px] font-bold" />}
                        {selfieUrl && <DocumentModal url={selfieUrl} label="ဆဲလ်ဖီ ကြည့်ရန်" className="px-2 py-1 bg-slate-100 text-slate-700 border border-slate-200 rounded text-[10px] font-bold" />}
                      </div>
                    )}
                  </div>
                  ...
                  <button className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors border shadow-sm ${
                    ...
                    {profile.is_blocked ? 'ပိတ်ပင်မှုရုတ်သိမ်းမည်' : 'ပိတ်ပင်မည်'}
                  </button>
                </div>
          ...
          <table className="w-full text-left text-sm text-slate-600">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 uppercase font-bold text-[11px] tracking-wider">
              <tr>
                ...
                <th className="px-6 py-4">အခန်းကဏ္ဍ</th>
                <th className="px-6 py-4">KYC အခြေအနေ</th>
                <th className="px-6 py-4">အကောင့်</th>
                  ...
                  <tr key={profile.id} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <div className="flex flex-col">
                          ...
                          {(profile.contact_app || profile.contact_username) && (
                            <span className="text-[11px] text-slate-500 mt-0.5 truncate max-w-[200px]">
                              <span className="font-bold">{profile.contact_app || 'ဆက်သွယ်ရန်'}:</span> {profile.contact_username}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-2 items-start">
                        ...
                        {(idCardUrl || selfieUrl) && (
                          <div className="flex gap-2">
                            {idCardUrl && <DocumentModal url={idCardUrl} label="မှတ်ပုံတင်" className="text-xs font-bold text-slate-500 hover:text-blue-600 underline" />}
                            {selfieUrl && <DocumentModal url={selfieUrl} label="ဆဲလ်ဖီ" className="text-xs font-bold text-slate-500 hover:text-blue-600 underline" />}
                          </div>
                        )}
                      </div>
                    </td>
